        ...
        if (thingtodo == "connect") {
                ...
                connect()
                    .then(() => {
                        const tableCreationQueries = [
                            `CREATE TABLE IF NOT EXISTS cadettrainings (
                                id INT AUTO_INCREMENT PRIMARY KEY, 
                                passed BOOL, 
                                cadet_username VARCHAR(64), 
                                cadet_id BIGINT, 
                                fto_username VARCHAR(64), 
                                fto_id BIGINT, 
                                timestamp TEXT
                            );`,
                            `CREATE TABLE IF NOT EXISTS departmentjoins (
                                id INT AUTO_INCREMENT PRIMARY KEY, 
                                forced BOOL, 
                                cadet_username VARCHAR(64), 
                                cadet_id BIGINT, 
                                department VARCHAR(64), 
                                admin_forced_username VARCHAR(64), 
                                admin_forced_id BIGINT, 
                                timestamp TEXT
                            );`,
                            ...
                            `CREATE TABLE IF NOT EXISTS pastrp (
                                id INT AUTO_INCREMENT PRIMARY KEY, 
                                aop VARCHAR(64), 
                                user TINYTEXT, 
                                timestamp TEXT, 
                        ...
                        const executeQuery = async (query: string) => {
                            return new Promise((resolve, reject) => {
                                mysqlconnection.query(query, function (err: any, result: unknown) {
                                    if (err) {
                                        console.error(err);
                                        reject(err);
                                    } else resolve(result);
                        ...
                        // Correctly wrapping the execution logic in an async IIFE (Immediately Invoked Function Expression)
                        (async () => {
                            try {
            ...
            connect()
                .then((result) => {
                    ...
                    if (table == "pastrp") {
                        valuestemplate = "(aop, timestamp, user, ping, training, pingatrptime)";
                    }
