...
export async function registerCommands() {
    ...
    let rules;
    try {
        rules = JSON.parse(fs.readFileSync(path.resolve(__dirname, "../../../config/rules.config.json"), "utf8"));
    } catch (error) {
    ...
    const commands = [
        ...
        },
        {
            ...
            options: [
                ...
                {
                    name: "time",
                    description: "When should the RP start. Accepts Unix Timestamps and relative time.",
                    type: 3,
                    required: true,
                },
                {
    ...
    try {
        ...
        await rest.put(Routes.applicationGuildCommands(env.parsed.CLIENT_ID, env.parsed.GUILD_ID), { body: commands });
...
// Run the function if the script is called directly by Node.js
