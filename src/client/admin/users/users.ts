import { announcement, closeAnnouncement, pageloaded } from "../../main";
                ...
                .then((data) => {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        ...
                        usershtml += `<tr>
                            ...
                            <td>${data[i].permission.text}</td>
                            ${isadmin.perm ? `<td class="buttons" id="buttons"><button id="editbtn" class="editbtn submitbtn" onclick="window.location.href = '/admin/users/edit?id=${data[i].id}';">Edit</button> <button id="${data[i].id}" class="submitbtn deletebtn" onclick="window.deleteUser(${data[i].id});">Delete</button></td>` : ""}
                        </tr>`;
                    }
