...
function pageloaded() {
    fetch(`/v1/pageload?l=${window.location.pathname}`, {
        // This chacks only the first request send to the page and does not request every a link is loaded.
        method: "GET",
    })
        .catch((error) => console.error("Error:", error))
        .then((response: Response | void) => response!.json())
        .then((data: { status?: string; redirect?: string }) => {
            if (data.redirect) {
    ...
    fetch("/header.json")
        ...
        .then((links) => {
            ...
            fetch("/v1/checkCookies?cookie=userid&perm=1", {
                ...
                .catch((error) => console.error("Error:", error))
                .then((response: Response | void) => response!.json())
                .then((data: { perm?: boolean; cookie?: boolean }) => {
